...
const SeatingForm = () => {
    ...
    const renderSeats = (rows, cols, section) => {
        // Calculate starting row number based on section
        let sectionStartRow;
        switch(section) {
            case 'Top': sectionStartRow = 1; break;
            case 'Left': sectionStartRow = config.topRows + 1; break;
        ...
        return (
            <div style={{ display: "grid", gap: "4px" }}>
                {[...Array(rows)]?.map((_, rowIndex) => {
                    ...
                    // Modified row label logic to include section identifier
                    let rowLabel;
                    if (section === 'Right') {
                        // Add 'R' prefix for right section
                        rowLabel = `R${String.fromCharCode(65 + sectionIndex)}${((totalRowNumber - 1) % 9) + 1}`;
                    } else if (section === 'Left') {
                        // Add 'L' prefix for left section
                        rowLabel = `L${String.fromCharCode(65 + sectionIndex)}${((totalRowNumber - 1) % 9) + 1}`;
                    } else {
                        // Keep original format for top and bottom sections
