...
const StarRating = memo(function StarRating({ count, count1 }) {

  // short trick 
  const stars = Array.from({ length: count })
  const stars1 = Array.from({ length: count1 })

  return (
    <>
      {stars.map((s, index) => {
        return (
          <svg key={index} className='icon-20' xmlns="http://www.w3.org/2000/svg" width="20" viewBox="0 0 24 24" fill="none">
